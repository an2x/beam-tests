...
public class InvalidWaitPipeline {

  private final static Logger LOG = LoggerFactory.getLogger(InvalidWaitPipeline.class);
  ...
  private static class WriteWrapper extends
    ...
    public PCollection<Integer> expand(PCollection<String> input) {
      POutput writeOutput = input.apply(new FakeNeo4jWriteUnwind(delay));
      return input.getPipeline().apply(Create.empty(BigEndianIntegerCoder.of()));
    }
  }

  private static class FakeNeo4jWriteUnwind extends PTransform<PCollection<String>, PDone> {
